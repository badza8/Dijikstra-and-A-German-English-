...
public class FibonacciHeap<T>
{
    ...
    //My Code (Not in Original!)
    FibonacciHeapNode<T> getNode(double x){
    	answer=null;
    	af.clear();
    	return getNode(x,minNode);
    }
    
    
    FibonacciHeapNode<T> answer=null;
    FibonacciHeapNode<T> getNode(double x,FibonacciHeapNode<T> y){
    	if(y==null) return answer;
    	
    	if(x == y.compID) {return y;}
    	if(answer!=null) return answer;
    	if(af.size()>nNodes) {return answer;}
    	
    	if(!af.contains(y)) af.add(y);
    	
    	if(y.child!=null) {
    		answer=getNode(x,y.child);
    	}
    	if(!af.contains(y.right)) {
    		answer=getNode(x,y.right);
    	}
    ...
    ArrayList<FibonacciHeapNode<T>> af=new ArrayList<FibonacciHeapNode<T>>();
